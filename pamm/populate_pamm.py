...
def populate():
    ...
    note_1 = add_user(title = 'Mystery plumes in Martian atmosphere baffle scientists',	
                author = user_pietro,
...
def add_note(title, author, timestamp, content, is_shared, tag):
    ...
    n = Note.objects.get_or_create(title = title, author = author, timestamp = timestamp, content = content, is_shared = is_shared, tag = tag)[0]
